...
const CoverStory = React.forwardRef<HTMLDivElement, CoverStoryProps>(
  ({ spotlight, issueLabel = "This Week's Feature", variant = "dark", className }, ref) => {
    ...
    return (
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr]">
          ...
          <div
            className={cn(
              "p-8 md:p-10 flex flex-col justify-center border-t md:border-t-0 md:border-l transition-colors duration-500",
              isLight
                ? "bg-ink-cream/50 border-ink-black/[0.06]"
                : "bg-ink-cream/[0.02] border-ink-cream/[0.04]"
            )}
          >
            <p
              className={cn(
                permanentMarker.className,
                "text-xs text-ink-red tracking-[0.15em] -rotate-1 inline-block mb-3"
              )}
            >
              {issueLabel}
            </p>
            <h3
              className={cn(
                bebasNeue.className,
                "text-3xl md:text-4xl tracking-wider leading-[1.1] transition-colors duration-500",
                isLight ? "text-ink-black" : "text-ink-cream"
              )}
            >
              {name}:
              <br />
              <span className={isLight ? "text-ink-black/50" : "text-ink-cream/60"}>
                {tagline}
              </span>
            </h3>
            <p
              className={cn(
                "text-sm leading-relaxed mt-4 transition-colors duration-500",
            ...
            <div className="flex flex-wrap gap-1.5 mt-4">
              {specialties.map((s) => (
                <span
                  key={s}
                  className={cn(
                    "px-3 py-1 rounded-full font-mono text-[9px] tracking-wide transition-colors duration-500",
                    isLight
