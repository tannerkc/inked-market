...
import type { HeroLayout, HeroCtaStyle } from "@/lib/types/builder";
  ...
  return (
    <div>
      ...
      <div className="mt-5 space-y-3">
        ...
        {/* Hero CTA Style selector (only when CTA is visible) */}
        {config.showHeroCta !== false && (
          <div>
            ...
            <div className="flex gap-1.5">
              {(["filled", "outline", "pill"] as HeroCtaStyle[]).map((style) => (
                <button
                  key={style}
                  type="button"
                  onClick={() => applyChange({ heroCtaStyle: style })}
                  className={cn(
                    "flex-1 rounded-lg border px-3 py-2 text-[11px] font-medium capitalize transition-colors",
                    (config.heroCtaStyle ?? "filled") === style
