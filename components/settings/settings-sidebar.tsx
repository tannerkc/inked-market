...
const SettingsSidebar = React.forwardRef<HTMLDivElement, SettingsSidebarProps>(
  ({ sections, activeSection, onSelect, className }, ref) => {
    ...
    return (
      <nav ref={ref} className={cn("hidden lg:block w-[200px] flex-shrink-0", className)}>
        <p className="font-mono text-[8px] tracking-[0.2em] uppercase mb-3 px-2.5 text-ink-black/25 dark:text-ink-cream/25">
          Settings
        </p>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-ink-black/[0.06] text-ink-black/80 dark:bg-ink-cream/[0.06] dark:text-ink-cream/80"
                    : "text-ink-black/45 hover:text-ink-black/60 hover:bg-ink-black/[0.03] dark:text-ink-cream/45 dark:hover:text-ink-cream/60 dark:hover:bg-ink-cream/[0.03]"
                )}
              >
                {section.label}
              </button>
        ...
        {dangerSections.length > 0 && (
          <>
            <div className="my-3 border-t border-ink-black/[0.04] dark:border-ink-cream/[0.04]" />
            <div className="space-y-0.5">
